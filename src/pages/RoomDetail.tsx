...
const RoomDetail = () => {
  ...
  usePageMeta(
    room ? `${room.name} — ${property.property_name}` : t("room.fallbackTitle"),
  ...
  if (loading) {
    return (
      <SiteLayout>
        <div className="container-narrow pt-40 pb-20 text-muted-foreground">{t("room.loading")}</div>
      </SiteLayout>
    );
  }
      ...
      <section className="relative h-[70vh] min-h-[480px] w-full overflow-hidden">
        ...
        <div className="relative container-narrow h-full flex flex-col justify-end pb-12 pt-32 text-background">
          <Link to="/rooms" className="inline-flex items-center gap-1.5 text-sm mb-4 opacity-85 hover:opacity-100">
            <ArrowLeft className="h-4 w-4" /> {t("room.allRooms")}
          </Link>
        ...
        <div className="container-narrow grid lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            <div className="flex flex-wrap gap-6 text-sm border-b border-border pb-6">
              <span className="inline-flex items-center gap-2"><Users className="h-4 w-4 text-primary" /> {room.capacity} {t("common.guests")}</span>
            ...
            {room.long_description && (
              <div>
                <p className="eyebrow mb-3">{t("room.about")}</p>
                <p className="text-lg leading-relaxed text-foreground/90">{room.long_description}</p>
              </div>
            )}
            {room.amenities.length > 0 && (
              <div>
                <p className="eyebrow mb-4">{t("room.amenities")}</p>
          ...
          <aside className="lg:sticky lg:top-28 h-fit rounded-lg border border-border bg-card p-6 md:p-8 shadow-card">
            ...
            <div className="mt-6 space-y-2 text-sm text-muted-foreground">
              <p>{t("room.checkInFrom", { time: property.checkin_time })}</p>
              <p>{t("room.checkOutBy", { time: property.checkout_time })}</p>
            </div>
            <Button asChild size="lg" className="w-full mt-6">
              <a href={property.booking_url} target="_blank" rel="noopener noreferrer">{t("common.bookThisRoom")}</a>
            </Button>
            <Button asChild size="lg" variant="outline" className="w-full mt-2">
              <Link to="/contact">{t("common.askQuestion")}</Link>
            </Button>
          </aside>
        </div>
      </section>
      ...
      {related.length > 0 && (
        <section className="section bg-secondary/40">
          <div className="container-narrow">
            <h2 className="text-3xl md:text-4xl mb-10">{t("room.youMightAlsoLike")}</h2>
